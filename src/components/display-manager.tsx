...
type DisplayIcon = 'usage' | 'alert' | 'battery' | 'wifi' | 'system'
type Document = { title: string; subtitle?: string; icon?: DisplayIcon; progress?: { value: number; max: number; label?: string; unit?: string }; lines?: Array<{ label: string; value: string }> }
...
export function DisplayManager() {
  ...
  const update_progress = (field: 'value' | 'max' | 'label' | 'unit', value: string) => {
    if (!active_page) return
    const current = active_page.document.progress ?? { value: 0, max: 100 }
    const next = field === 'value' || field === 'max' ? { ...current, [field]: Number(value) || 0 } : { ...current, [field]: value }
    set_pages(pages.map((page) => page.page_id === active_page.page_id ? { ...page, document: { ...page.document, progress: next } } : page))
  }
  const preview = async () => {
  ...
  return <main className="sources-shell"><header className="dashboard-header"><Flexbox className="dashboard-introduction" gap={10}><Button icon={ArrowLeft} onClick={() => router.push('/')} size="large">{translate('back')}</Button><Text className="eyebrow"><Monitor aria-hidden />{translate('eyebrow')}</Text><h1>{translate('title')}</h1><Text className="header-subtitle">{translate('subtitle')}</Text></Flexbox><Segmented aria-label={translate('language')} options={[{ label: 'EN', value: 'en' }, { label: '中文', value: 'zh-CN' }, { label: '日本語', value: 'ja' }]} value={locale} onChange={(value) => change_locale(value as 'en' | 'zh-CN' | 'ja')} /></header>
    <section className="display-editor-grid" aria-label={translate('editor')}><Block className="display-editor" variant="outlined"><Flexbox horizontal align="center" justify="space-between" gap={8}><h2>{translate('editor')}</h2><Button icon={Plus} disabled={pages.length >= 10} onClick={add_page}>{translate('addPage')}</Button></Flexbox><Flexbox className="page-tabs" horizontal gap={8} wrap="wrap">{pages.map((page) => <Button key={page.page_id} onClick={() => set_active_page_id(page.page_id)} type={page.page_id === active_page_id ? 'primary' : 'default'}>{page.page_id}</Button>)}</Flexbox>{active_page && <Flexbox className="source-form" gap={8}><label htmlFor="display-page-id">{translate('pageId')}</label><Input id="display-page-id" pattern="[a-z0-9-]+" value={active_page.page_id} onChange={(event) => { const next_page_id = event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-'); set_pages(pages.map((page) => page.page_id === active_page.page_id ? { ...page, page_id: next_page_id } : page)); set_active_page_id(next_page_id) }} /><label htmlFor="display-title">{translate('displayTitle')}</label><Input id="display-title" maxLength={48} required value={active_page.document.title} onChange={(event) => update_document('title', event.target.value)} /><label htmlFor="display-subtitle">{translate('displaySubtitle')}</label><Input id="display-subtitle" maxLength={80} value={active_page.document.subtitle ?? ''} onChange={(event) => update_document('subtitle', event.target.value)} /><label>{translate('displayIcon')}</label><Segmented options={[{ label: translate('iconNone'), value: 'none' }, { label: '↗', value: 'usage' }, { label: '!', value: 'alert' }, { label: '▣', value: 'battery' }, { label: ')))', value: 'wifi' }, { label: '▤', value: 'system' }]} value={active_page.document.icon ?? 'none'} onChange={(value) => update_document('icon', value === 'none' ? '' : value)} /><Checkbox checked={Boolean(active_page.document.progress)} onChange={(checked) => { if (!active_page) return; set_pages(pages.map((page) => page.page_id === active_page.page_id ? { ...page, document: { ...page.document, progress: checked ? { value: 0, max: 100, label: translate('progressLabel'), unit: translate('progressUnit') } : undefined } } : page)) }}>{translate('enableProgress')}</Checkbox>{active_page.document.progress && <Flexbox horizontal gap={8} wrap="wrap"><Input aria-label={translate('progressValue')} min={0} type="number" value={active_page.document.progress.value} onChange={(event) => update_progress('value', event.target.value)} /><Input aria-label={translate('progressMax')} min={1} type="number" value={active_page.document.progress.max} onChange={(event) => update_progress('max', event.target.value)} /><Input aria-label={translate('progressLabel')} value={active_page.document.progress.label ?? ''} onChange={(event) => update_progress('label', event.target.value)} /><Input aria-label={translate('progressUnit')} value={active_page.document.progress.unit ?? ''} onChange={(event) => update_progress('unit', event.target.value)} /></Flexbox>}<label htmlFor="display-lines">{translate('displayLines')}</label><TextArea id="display-lines" rows={8} value={lines_text} onChange={(event) => update_lines(event.target.value)} /><Text type="secondary">{translate('linesHint')}</Text><Button icon={Trash2} disabled={pages.length <= 1} onClick={() => remove_page(active_page.page_id)}>{translate('removePage')}</Button></Flexbox>}</Block><Block className="display-preview-card" variant="outlined"><Flexbox horizontal align="center" justify="space-between"><h2>{translate('preview')}</h2><Button icon={Eye} loading={preview_loading} onClick={() => void preview()}>{translate('refreshPreview')}</Button></Flexbox>{preview_svg ? <img alt={translate('previewAlt')} className="display-editor-preview" src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(preview_svg)}`} /> : <Empty emoji="▧" title={translate('previewEmpty')} description={translate('previewEmptyDescription')} />}</Block></section>

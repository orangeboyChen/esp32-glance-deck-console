...
const otaSchema = z.object({ firmware_release_id: z.uuid().optional() })
...
export const POST = async (request: Request, { params }: { params: Promise<{ device_id: string }> }) => {
  ...
  const body = otaSchema.safeParse(await request.json())
  ...
  const [previousRelease] = device.firmware_version
    ? await db
        ...
        .limit(1)
    : []
  if (previousRelease) await db.update(devices).set({ last_good_firmware_release_id: previousRelease.id }).where(eq(devices.id, deviceId))
  const [job] = await db
    .insert(otaJobs)

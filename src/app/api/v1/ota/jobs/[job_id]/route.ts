...
export const PATCH = async (request: Request, { params }: { params: Promise<{ job_id: string }> }) => {
  ...
  const body = otaJobRequestSchema.safeParse(await request.json().catch(() => ({})))
  if (!body.success) {
    return NextResponse.json({ error: 'invalid_ota_job_action' }, { status: 400 })
  }
  if (body.data.action === 'rollback') {
    const [job] = await db
      ...
      .limit(1)
    if (!job?.release_id) {
      return NextResponse.json({ error: 'known_good_release_not_found' }, { status: 409 })
    }

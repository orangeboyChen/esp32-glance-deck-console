...
export const deviceStatus = pgEnum('device_status', ['enrolling', 'online', 'offline', 'error'])
export const commandStatus = pgEnum('command_status', ['queued', 'sent', 'confirmed', 'failed'])
export const sourceStatus = pgEnum('source_status', ['active', 'paused', 'error', 'refreshing'])
export const otaJobStatus = pgEnum('ota_job_status', [
...
export const alertOperator = pgEnum('alert_operator', ['gt', 'gte', 'lt', 'lte', 'eq', 'neq', 'contains'])
...
export const sessions = pgTable(
  ...
  {
    id: uuid('id').defaultRandom().primaryKey(),
    administrator_id: uuid('administrator_id')
...
export const displayReleases = pgTable('display_releases', {
  id: uuid('id').defaultRandom().primaryKey(),
...
export const displayReleasePages = pgTable(
...
export const usageSources = pgTable('usage_sources', {
  id: uuid('id').defaultRandom().primaryKey(),
  ...
  status: sourceStatus('source_status').default('active').notNull(),
...
export const alertRules = pgTable('alert_rules', {
  ...
  operator: alertOperator('operator').notNull(),
...
export const displayBindings = pgTable('display_bindings', {
  id: uuid('id').defaultRandom().primaryKey(),
  source_id: uuid('source_id')
    .references(() => usageSources.id, { onDelete: 'cascade' })
    .notNull(),
  page_id: varchar('page_id', { length: 64 }).notNull(),
  document_template: jsonb('document_template')
    .$type<{
      title: string
      subtitle?: string
...
export const devices = pgTable('devices', {
  ...
  status: deviceStatus('status').default('enrolling').notNull(),
  firmware_version: varchar('firmware_version', { length: 64 }),
  last_good_firmware_release_id: uuid('last_good_firmware_release_id').references(() => firmwareReleases.id),
  ...
  release_id: uuid('release_id').references(() => displayReleases.id),
...
export const otaJobs = pgTable('ota_jobs', {
  id: uuid('id').defaultRandom().primaryKey(),
  device_id: varchar('device_id', { length: 64 })
    .references(() => devices.id, { onDelete: 'cascade' })
    .notNull(),
  firmware_release_id: uuid('firmware_release_id')
    .references(() => firmwareReleases.id)
    .notNull(),
  status: otaJobStatus('ota_job_status').default('queued').notNull(),

...
import { normalizeSoruxgptCodex, normalizeSoruxgptToken, publicSoruxgptSource } from '@/server/source/soruxgpt'

describe('SoruxGPT Codex usage normalization', () => {
  test('aggregates active quota windows and converts USD micro-units', () => {
    expect(
      normalizeSoruxgptCodex(
        {
          display_name: 'Codex',
          usage_limits: [
            {
              current_usage: 100_000_000,
              limit_value: 388_000_000,
              limit_type: 'usd',
              time_unit: 'day',
              time_value: 1,
            ...
            {
              current_usage: 837_000_000,
              limit_value: 1_027_000_000,
              limit_type: 'usd',
              time_unit: 'week',
              time_value: 1,
              next_available_at: '2026-08-16T00:00:00Z',
            },
          ],
        },
      ...
      plan_name: 'SoruxGPT Codex',
      used: 937,
      remaining: 478,
      total: 1415,
      unit: 'USD',
      resets_at: '2026-08-16T00:00:00Z',
      status: '2 active quota windows',
    })
  })

  test('rejects responses without usable quota windows', () => {
    expect(() => normalizeSoruxgptCodex({ usage_limits: [] })).toThrow('soruxgpt_usage_limits_missing')
  })
  ...
  test('ignores an expired window even when it has the higher consumption ratio', () => {
    ...
    ).toMatchObject({ used: 50, total: 100 })
  })
  ...
  test('rejects active limits with incompatible units', () => {
    ...
    ).toThrow('soruxgpt_usage_limits_mixed_units')
  })

  test('rejects expired limits with malformed expiration timestamps', () => {
    expect(() =>
      normalizeSoruxgptCodex(
        { usage_limits: [{ current_usage: 1, limit_value: 2, expires_at: 'not-a-date' }] },
        new Date('2026-08-15T00:00:00Z'),
      ),
    ).toThrow('soruxgpt_usage_limits_missing')
  })

  test('omits malformed reset timestamps', () => {
    expect(
      normalizeSoruxgptCodex(
        { usage_limits: [{ current_usage: 1, limit_value: 2, next_available_at: 'not-a-date' }] },
        new Date('2026-08-15T00:00:00Z'),
      ).resets_at,
    ).toBeNull()
  })
...
describe('SoruxGPT source credentials', () => {
  test('normalizes a raw token and tolerates an accidental Bearer prefix', () => {
    expect(normalizeSoruxgptToken('  raw-token  ')).toBe('raw-token')
    expect(normalizeSoruxgptToken('Bearer raw-token')).toBe('raw-token')
    expect(normalizeSoruxgptToken('bearer   raw-token  ')).toBe('raw-token')
  })

  test('projects only public source fields', () => {
    const source = { id: 'source-id', name: 'SoruxGPT Codex', secret_ciphertext: 'encrypted' }
    expect(publicSoruxgptSource(source)).toEqual({ id: 'source-id', name: 'SoruxGPT Codex' })
  })
})

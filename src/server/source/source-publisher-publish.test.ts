...
describe('source-bound display publication', () => {
  ...
  test('creates a new bitmap release and publishes it to every assigned device', async () => {
    ...
    expect(
      await publishSourceChanges('source-1', { plan_name: 'Pro', used: 72 }, { database: database as never, publishRelease: published }),
    ).toBe(2)
  ...
  test('does not republish an unchanged release already assigned to all devices', async () => {
    ...
    expect(await publishSourceChanges('source-1', {}, { database: database as never, publishRelease: published })).toBe(0)
  ...
  test('rejects a binding that references missing devices', async () => {
    ...
    await expect(publishSourceChanges('source-1', {}, { database: database as never, publishRelease: published })).rejects.toThrow(
      'display_binding_device_not_found',
    )
  })
})

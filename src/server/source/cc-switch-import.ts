...
const redactValue = (value: unknown, key?: string): unknown => {
  ...
  if (typeof value === 'string' && /\b(?:bearer|basic)\s+\S+/i.test(value)) {
    return '[REDACTED]'
  }
  return value
}
...
const variablesIn = (value: unknown) => {
  ...
  const collect = (item: unknown) => {
    if (typeof item === 'string') {
      for (const match of item.matchAll(variableName)) {
...
const extractorTargets = (extractor: string | undefined) => {
  ...
  const names = new Set<string>()
  for (const match of returned.matchAll(/(?:^|,)\s*(?:([A-Za-z_$][\w$]*)|['"]([^'"]+)['"])\s*:/g)) {
    names.add(match[1] || match[2])
  }
  return [...names].sort()
}
...
export const previewCcSwitchImport = (exported: JsonValue): CcSwitchImportPreview => {
  const serialized = JSON.stringify(exported)
  if (!serialized || serialized.length > MAX_EXPORT_BYTES) {
    throw new Error('cc_switch_export_too_large')
  }
  const root = record(exported)
  if (!root) {
    throw new Error('cc_switch_export_invalid')
  }
  const request = requestFrom(root)
  const url = string(request.url) ?? string(request.request_url) ?? string(root.url)
  if (!url) {
    throw new Error('cc_switch_request_url_missing')
  }
  let parsed: URL
  try {
    parsed = new URL(url)
  ...
  if (methodValue !== 'GET' && methodValue !== 'POST') {
    throw new Error('cc_switch_request_method_invalid')
  }
  ...
  const body = (request.body ?? request.data ?? request.body_template ?? null) as JsonValue
  ...
  return {
    body: redactValue(body) as JsonValue,

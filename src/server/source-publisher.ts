...
import { db } from './db'
import { publishDeviceRelease } from './mqtt'
import { MONO1_IMAGE_FORMAT, renderDeviceBitmap, type DisplayDocument } from './preview'
import { devices, displayBindings, displayReleasePages, displayReleases } from './schema'
...
export const publishSourceChanges = async (sourceId: string, values: SourceValues) => {
  if (!db) return 0
  const bindings = await db.select().from(displayBindings).where(eq(displayBindings.source_id, sourceId))
  let published = 0

...
const outputDirectory = resolve(import.meta.dirname, '../../docs/image2')
const width = 1600
const height = 1024

const escapeXml = (value) =>
  value.replace(/[<>&"']/g, (character) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' })[character])
const text = (value, x, y, size = 20, weight = 400, color = '#1d1d1f', anchor = 'start') =>
  `<text x="${x}" y="${y}" text-anchor="${anchor}" font-family="Arial, sans-serif" font-size="${size}" font-weight="${weight}" fill="${color}">${escapeXml(value)}</text>`
const rect = (x, y, boxWidth, boxHeight, fill = '#ffffff', stroke = '#d2d2d7', radius = 16) =>
  `<rect x="${x}" y="${y}" width="${boxWidth}" height="${boxHeight}" rx="${radius}" fill="${fill}" stroke="${stroke}"/>`
const rule = (x1, y1, x2, y2) => `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#d2d2d7"/>`
const button = (label, x, y, boxWidth, primary = false) =>
  `${rect(x, y, boxWidth, 42, primary ? '#0b7a5c' : '#ffffff', primary ? '#0b7a5c' : '#c7c7cc', 11)}${text(label, x + boxWidth / 2, y + 28, 16, 600, primary ? '#ffffff' : '#1d1d1f', 'middle')}`
const badge = (label, x, y, color = '#e8f5e9') =>
  `${rect(x, y, 96, 30, color, color, 15)}${text(label, x + 48, y + 21, 13, 700, '#1d1d1f', 'middle')}`
const input = (label, x, y, boxWidth, value = '') =>
  `${text(label, x, y - 10, 14, 600, '#3a3a3c')}${rect(x, y, boxWidth, 44, '#ffffff', '#c7c7cc', 9)}${value ? text(value, x + 14, y + 29, 16, 400, '#6e6e73') : ''}`
const sectionHeading = (label, x, y, detail = '') =>
  `${text(label, x, y, 25, 700)}${detail ? text(detail, x, y + 25, 15, 400, '#6e6e73') : ''}`
const iconChoice = (label, x, y, selected = false) =>
  `${rect(x, y, 52, 42, selected ? '#007aff' : '#ffffff', selected ? '#007aff' : '#c7c7cc', 11)}${text(label, x + 26, y + 28, 14, 700, selected ? '#ffffff' : '#1d1d1f', 'middle')}`

const frame = (title, subtitle, body) => {
...
const dashboard = () => {
  ...
  return frame(
    'Devices at a glance',
    'A device-accurate view of displays, alerts, and Home Assistant automation.',
    `
    ...
    ${sectionHeading('Devices', 88, 450, 'Select a device to inspect its last confirmed page.')}
    ...
    ${sectionHeading('Display pages', 88, 830, 'Shown after selecting a device.')}
...
const displays = () => {
  return frame(
    'Display releases',
    'Compose fixed 400 × 300 pages, inspect the device-accurate preview, and publish an immutable release.',
    `
    ${sectionHeading('Page editor', 88, 256)}${button('+ Add page', 570, 226, 132)}${sectionHeading('Device preview', 858, 256)}${button('Refresh preview', 1166, 226, 170)}
    ...
    ${text('Icon', 112, 644, 14, 600, '#3a3a3c')}${button('None', 112, 656, 76)}${iconChoice('Usage', 200, 656, true)}${iconChoice('Home', 264, 656)}${iconChoice('Battery', 328, 656)}${iconChoice('Wi-Fi', 392, 656)}${iconChoice('System', 456, 656)}
    ...
    ${sectionHeading('Publish targets', 88, 900, 'Select devices, then open the publish confirmation.')}${rect(88, 940, 1248, 38, '#fbfbfd')}${text('☐ Kitchen display', 118, 966, 16, 600)}${button('Publish display', 1128, 936, 176, true)}
  `,
  )
}
...
const sources = () => {
  return frame(
    'Usage sources',
    'Collect subscription and status data, then bind mapped values to display documents.',
    `
    ${sectionHeading('SoruxGPT Codex quota', 88, 246, 'Connect once; the control plane stores the token encrypted and refreshes the live quota.')}${rect(88, 274, 1248, 136, '#fbfbfd')}${input('SoruxGPT access token', 112, 320, 736, '••••••••••••')}${button('Connect SoruxGPT', 876, 320, 190, true)}${text('Most constrained active day, week, or month window drives the usage meter.', 112, 390, 15, 400, '#6e6e73')}
    ${sectionHeading('Saved sources', 88, 468)}${button('Refresh', 1204, 438, 132)}${rect(88, 496, 1248, 108, '#fbfbfd')}${text('SoruxGPT Codex', 116, 540, 22, 700)}${badge('ACTIVE', 324, 518)}${text('GET app.soruxgpt.com/api/v1/codex · refreshes every 900 seconds', 116, 576, 16, 400, '#6e6e73')}${button('Test request', 1166, 532, 140)}
    ${sectionHeading('Import a CC Switch export', 88, 660, 'Review request settings without executing extractor JavaScript.')}${rect(88, 694, 1248, 106, '#fbfbfd')}${text('Paste UsageScript JSON export…', 112, 738, 17, 400, '#8e8e93')}${button('Review import', 112, 752, 144)}
    ${sectionHeading('Create source', 88, 858, 'Secrets are encrypted after saving; additional fields continue below.')}${rect(88, 884, 760, 94, '#fbfbfd')}${input('Name', 112, 914, 336, 'New usage source')}${input('Base URL', 472, 914, 352, 'https://api.example.com')}
  `,
  )
}
...
const alerts = () => {
  return frame(
    'Alerts',
    'Evaluate source values and send a clear alert page to selected devices.',
    `
    ${sectionHeading('Alert rules', 88, 258)}${button('Refresh', 1204, 228, 132)}${rect(88, 286, 1248, 132, '#fbfbfd')}${text('Low remaining balance', 116, 330, 22, 700)}${badge('ACTIVE', 368, 308, '#ffe5e5')}${badge('ENABLED', 476, 308)}${text('Codex usage · remaining · at most 20 · 1 device / usage', 116, 370, 16, 400, '#6e6e73')}${button('Delete', 1180, 338, 126)}
    ${sectionHeading('Create alert rule', 88, 484, 'Test-only rules never control a device.')}${rect(88, 510, 760, 466, '#fbfbfd')}
...
const firmware = () => {
  return frame(
    'Firmware releases',
    'Install a verified release only on compatible devices.',
    `
    ${sectionHeading('Verified releases', 88, 258)}${button('Refresh', 1204, 228, 132)}${rect(88, 286, 1248, 282, '#fbfbfd')}${text('v0.1.0', 116, 336, 26, 700)}${text('ESP32-S3-RLCD-4.2', 116, 368, 16, 400, '#6e6e73')}${badge('STABLE', 320, 314)}${text('Verified Aug 13 · View manifest', 116, 408, 16, 400, '#6e6e73')}${rule(116, 438, 1308, 438)}${text('Kitchen display', 116, 486, 18, 600)}${text('Current: 0.1.0 · Update: healthy', 116, 516, 16, 400, '#6e6e73')}${button('Start update', 1044, 474, 140, true)}${button('Roll back', 1196, 474, 110)}
    ${sectionHeading('Staged rollout', 88, 644, 'Queue a signed release for a percentage of power-safe compatible devices.')}${rect(88, 674, 650, 302, '#fbfbfd')}
...
for (const [name, svg] of Object.entries({
  ...
  await writeFile(resolve(outputDirectory, `${name}.png`), await sharp(Buffer.from(svg)).png().toBuffer())
}
